...
for (i in 21:27) {
  for (j in seq(0, 18, by = 6)) {
    if (j < 10) {
      j <- (paste("0", j, sep = ""))
    }
    vec <- strsplit(paste(i, j), split = " ")
    test <- unlist(vec)
    
    scrapeForecast(test[1], test[2])
  }
}

scrapeForecast <- function(day, hour) {
  
  forecast.time <- paste("12/", day, "/2012 ", hour, ":00:00 UTC", sep = "")
  url <- paste("http://wxweb.meteostar.com/sample/archive.shtml?text=KSEA&run=201212", day, hour, sep = "")
  tbl <- readHTMLTable(url)
